...
class WorkerController extends Controller
{
    ...
    private function generateQrFile(Worker $worker)
    {
        // ID de la tabla workers
        $workerId = $worker->id;

        // Matrícula institucional
        $institutionalId = $worker->id_institucional;
        ...
        $hash = hash( 'sha256', "{$token}_{$workerId}_" . Carbon::now()->toIso8601String());
        ...
        // URL QR
        $qrData ="https://registroescolar.uacm.edu.mx/scan?token={$token}&sig=". hash('sha256', "{$workerId}_{$type}");
