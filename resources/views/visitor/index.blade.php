                            ...
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach ($visitors as $visitor)
										...
										<td >{{ $visitor->code_qr }}</td>
										<td >{{ $visitor->reactivacion }}</td>
										<td >{{ $visitor->fechas_impresion }}</td>

                                            <td>
                                                <form action="{{ route('visitor.destroy', $visitor->id_visitor) }}" method="POST">
                                                    <a class="btn btn-sm btn-primary " href="{{ route('visitor.show', $visitor->id_visitor) }}"><i class="fa fa-fw fa-eye"></i> {{ __('Show') }}</a>
                                                    <a class="btn btn-sm btn-success" href="{{ route('visitor.edit', $visitor->id_visitor) }}"><i class="fa fa-fw fa-edit"></i> {{ __('Edit') }}</a>
                                                    @csrf
                                                    @method('DELETE')
